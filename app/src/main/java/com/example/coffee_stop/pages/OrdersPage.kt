...
import com.example.coffee_stop.ItemInCart
import com.example.coffee_stop.Product
import com.example.coffee_stop.ui.theme.Primary

@Composable
fun OrdersPage(dataHandler: DataHandler) {
    LazyColumn() {
        if (dataHandler.cart.isEmpty()) {
            item {
                Card(
                    modifier = Modifier
                        .padding(16.dp)
        ...
        items(dataHandler.cart) { it ->
            CartItem(it, onDelete = {
                dataHandler.cartRemove(it)
            })
        }
    }
}
